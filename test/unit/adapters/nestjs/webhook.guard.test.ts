...
import { HOOKSENTINEL_PROVIDER_METADATA_KEY } from '../../../../src/adapters/nestjs/tokens.js';
import type { HooksentinelHttpRequest } from '../../../../src/adapters/nestjs/types.js';
...
describe('WebhookGuard — branches not reachable through the full-app integration suites', () => {
  ...
  it('falls back to req.url when req.originalUrl is absent (Fastify-shaped request)', async () => {
    ...
    Reflect.defineMetadata(HOOKSENTINEL_PROVIDER_METADATA_KEY, 'stripe', handler);
    ...
    const req: HooksentinelHttpRequest = {
    ...
    expect(req.hooksentinelPending?.event.id).toBe('evt_1');
  });
});
